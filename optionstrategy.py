...
# =================================================================
#  SECTION 1 -- NSE OPTION CHAIN FETCHER  (ALL STRIKES, no filter)
# =================================================================
...
class NSEOptionChain:
    ...
    def _upcoming_thursday(self):
        """NSE weekly expiry is Thursday."""
        ist_tz    = pytz.timezone("Asia/Kolkata")
        today_ist = datetime.now(ist_tz).date()
        weekday   = today_ist.weekday()          # Mon=0 … Sun=6
    ...
    def _fetch_for_expiry(self, session, headers, expiry):
        ...
        for attempt in range(1, 3):
            try:
                ...
                print(f"    OK {len(df)} strikes fetched (ALL strikes, no filter)")
                return {
                    "expiry":     expiry,
                    "df":         df,
    ...
    def fetch(self):
        ...
        expiry = self._upcoming_thursday()
...
# =================================================================
#  SECTION 2 -- OPTION CHAIN ANALYSIS  (uses full df)
# =================================================================
...
def analyze_option_chain(oc_data):
    ...
    if   ce_chg > 0 and pe_chg < 0:
        oi_dir, oi_sig, oi_cls = "Strong Bearish", "Call Build-up + Put Unwinding",    "bearish"
    elif ce_chg < 0 and pe_chg > 0:
        oi_dir, oi_sig, oi_cls = "Strong Bullish", "Put Build-up + Call Unwinding",    "bullish"
    elif ce_chg > 0 and pe_chg > 0:
        if   pe_chg > ce_chg * 1.5:
            oi_dir, oi_sig, oi_cls = "Bullish",            "Put Build-up Dominant",    "bullish"
        elif ce_chg > pe_chg * 1.5:
            oi_dir, oi_sig, oi_cls = "Bearish",            "Call Build-up Dominant",   "bearish"
        else:
            oi_dir, oi_sig, oi_cls = "Neutral (High Vol)", "Both Calls & Puts Building","neutral"
    elif ce_chg < 0 and pe_chg < 0:
        oi_dir, oi_sig, oi_cls = "Neutral (Unwinding)", "Both Calls & Puts Unwinding", "neutral"
    else:
        if   net_chg > 0: oi_dir, oi_sig, oi_cls = "Moderately Bullish", "Net Put Accumulation",  "bullish"
        elif net_chg < 0: oi_dir, oi_sig, oi_cls = "Moderately Bearish", "Net Call Accumulation", "bearish"
        else:              oi_dir, oi_sig, oi_cls = "Neutral",            "Balanced OI Changes",   "neutral"
...
def compute_market_direction(tech, oc_analysis):
    if not tech:
        return {"bias": "UNKNOWN", "bias_cls": "neutral", "confidence": "LOW", "bull": 0, "bear": 0, "diff": 0}
...
# =================================================================
#  SECTION 6 -- STRATEGY DEFINITIONS  (full 37-strategy library)
# =================================================================
...
ALL_STRATEGIES = {
    ...
    "nondirectional": {
        ...
        "items": [
            ...
            {"name": "Double Condor",         "risk": "Limited",   "reward": "Limited",   "legs": "TWO CONDOR SPREADS combined",                             "desc": "Extended profit zone of condor. Very wide range strategy.",                                           "mp": "Net Credit",             "ml": "Wing - Credit",       "be": "Outer short strikes ± Cr.","payoff": "double_condor"},
            ...
            {"name": "Diagonal Calendar",     "risk": "Limited",   "reward": "Limited",   "legs": "SELL Near-Term · BUY Far-Term (Different Strike)",         "desc": "Like calendar spread but with different strikes. More flexibility.",                                  "mp": "Varies",                 "ml": "Net Debit",           "be": "Varies",                   "payoff": "diagonal_calendar"},
...
def build_oi_html(oc):
    ...
    oi_cls = oc["oi_cls"];  oi_dir = oc["oi_dir"];  oi_sig = oc["oi_sig"];  expiry = oc["expiry"]
    n_str  = oc.get("total_strikes", "All")
    ...
    return (
        ...
        f"<span class='sec-sub'>All {n_str} strikes &middot; Expiry: {expiry}</span></div>"
...
def build_key_levels_html(tech, oc):
    ...
    mp_html = ""
    if oc:
        mp_p = pct(oc["max_pain"]);  max_pain = oc["max_pain"]
        mp_html = (
            ...
            f"<div class='kl-val' style='color:#b8880e;'>&#8377;{max_pain:,}</div>"
            f"</div>"
        )
    ...
    return (
        ...
        f"<div class='kl-val' style='color:#0d4a5a;'>&#8377;{ss:,.0f}</div>"
        ...
        f"<div class='kl-val' style='color:#0d5c6e;'>&#8377;{s1:,.0f}</div>"
        ...
        f"<div class='kl-val' style='color:#6b1c12;'>&#8377;{sr:,.0f}</div>"
...
def build_strategy_selector_html(md, tech, oc):
    ...
    tab_defs = [("bullish","BULLISH","#0fa86b","9"),("bearish","BEARISH","#c0392b","8"),("nondirectional","NON-DIRECTIONAL","#7b6cf6","20")]
    tabs_html = '<div class="stab-wrap">' + "".join(
        f'<button class="stab {"stab-active" if k=="bullish" else ""}" data-tab="{k}" '
        f'onclick="switchTab(\'{k}\')" style="--tc:{c};">'
        f'{l} <span class="stab-count">{n}</span></button>'
        for k, l, c, n in tab_defs
    ) + "</div>"
    ...
    for cat_key, cat_data in ALL_STRATEGIES.items():
        ...
        cards   = "".join(
            ...
            f'<div class="sc-svg">{get_payoff_svg(s.get("payoff",""), 80, 50)}</div>'
            f'<div class="sc-name">{s["name"]}</div>'
            f'<div class="sc-legs">{s["legs"]}</div>'
            f'<div class="sc-badges">'
            f'<span class="sc-badge" style="color:{"#0fa86b" if s["risk"]=="Limited" else "#d4a017" if s["risk"]=="Moderate" else "#c0392b"};">Risk: {s["risk"]}</span>'
            f'<span class="sc-badge" style="color:{"#0fa86b" if s["reward"]=="Unlimited" else "#d4a017" if s["reward"]=="High" else "#8a8a8a"};">Reward: {s["reward"]}</span>'
            f'</div></div>'
            for s in cat_data["items"]
        )
        panels_html += f'<div id="tab-{cat_key}" class="stab-panel" style="display:{visible};"><div class="strat-grid">{cards}</div></div>'

    modal_html = """
<div id="strat-modal" class="smodal-overlay" onclick="if(event.target===this)closeModal()">
  <div class="smodal-box">
    <div class="smodal-header">
      <div><div class="smodal-name" id="modal-name"></div><div class="smodal-legs" id="modal-legs"></div></div>
...
function switchTab(key){
  ...
  const p=document.getElementById('tab-'+key); if(p) p.style.display='block';
  const b=document.querySelector('[data-tab="'+key+'"]'); if(b) b.classList.add('stab-active');
}
function openModal(card){
  ...
  const svg=card.querySelector('.sc-svg');
  document.getElementById('modal-svg').innerHTML=svg ? svg.innerHTML.replace('width="80" height="50"','width="220" height="130"') : '';
  document.getElementById('strat-modal').style.display='flex';
}
function closeModal(){ document.getElementById('strat-modal').style.display='none'; }
document.addEventListener('keydown',e=>{ if(e.key==='Escape') closeModal(); });
    ...
    return (
        ...
        f"{tabs_html}{panels_html}{modal_html}</div>"
    )


def build_strikes_html(oc):
    """Top 10 strikes by OI — ALL strikes in scope, no ATM filter."""
    ...
    def ce_rows(rows):
        ...
        for i, r in enumerate(rows, 1):
            chg    = int(r.get("CE_OI_Change", 0))
            chg_c  = "#0fa86b" if chg < 0 else "#c0392b" if chg > 0 else "#8a8a8a"
            chg_s  = f"{chg:+,}"
            out += (
                f"<tr>"
                f"<td>{i}</td>"
                f"<td><b>&#8377;{int(r['Strike']):,}</b></td>"
                f"<td>{int(r['CE_OI']):,}</td>"
                f"<td style='color:{chg_c};font-size:11px;'>{chg_s}</td>"
                f"<td>{int(r.get('CE_Vol',0)):,}</td>"
                f"<td style='color:#0d8a9e;font-weight:700;'>&#8377;{r['CE_LTP']:.2f}</td>"
                f"</tr>"
            )
        return out
    ...
    def pe_rows(rows):
        ...
        for i, r in enumerate(rows, 1):
            ...
            out += (
                ...
                f"<td>{int(r['PE_OI']):,}</td>"
                f"<td style='color:{chg_c};font-size:11px;'>{chg_s}</td>"
                f"<td>{int(r.get('PE_Vol',0)):,}</td>"
                f"<td style='color:#c0392b;font-weight:700;'>&#8377;{r['PE_LTP']:.2f}</td>"
                f"</tr>"
            )
        return out
    ...
    return (
        ...
        f"<table class='s-table'>"
        f"<thead><tr><th>#</th><th>Strike</th><th>OI</th><th>OI Chg</th><th>Volume</th><th>LTP</th></tr></thead>"
        f"<tbody>{ce_rows(oc['top_ce'])}</tbody></table></div>"
        f"<div>"
        f"<div style='color:#c0392b;font-weight:700;margin-bottom:10px;'>PUT Options (PE)</div>"
        f"<table class='s-table'>"
        f"<thead><tr><th>#</th><th>Strike</th><th>OI</th><th>OI Chg</th><th>Volume</th><th>LTP</th></tr></thead>"
...
header{display:flex;align-items:center;justify-content:space-between;padding:14px 32px;
  ...
  background:linear-gradient(135deg,#0d4a5a,#0d6e80);position:sticky;top:0;z-index:200;
...
.hero{padding:26px 32px;background:linear-gradient(135deg,#0d4a5a 0%,#0d6e80 50%,#1a9aad 100%);
  border-bottom:1px solid rgba(255,255,255,.1);display:flex;align-items:center;
  justify-content:space-between;gap:24px;flex-wrap:wrap;box-shadow:0 3px 16px rgba(13,74,90,.2)}
...
.hero-conf{display:inline-block;margin-top:10px;font-size:11px;font-weight:600;
...
.sidebar{border-right:1px solid var(--bdr2);background:linear-gradient(180deg,#d0dae6 0%,#c8d4e2 100%);
...
.sb-lbl{font-size:9px;font-weight:700;letter-spacing:.15em;text-transform:uppercase;
  color:var(--teal);margin-bottom:8px;padding:0 0 0 8px;border-left:3px solid var(--teal)}
...
.sig-card{margin:12px 10px 8px;padding:16px 14px;
  background:linear-gradient(135deg,#0d4a5a,#0d8a9e);border-radius:12px;
  border:1px solid rgba(255,255,255,.1);text-align:center;box-shadow:0 4px 16px rgba(13,74,90,.25)}
...
.sec-title{font-family:var(--fh);font-size:12px;font-weight:700;letter-spacing:2px;
  color:var(--teal-dark);text-transform:uppercase;display:flex;align-items:center;
  gap:10px;flex-wrap:wrap;margin-bottom:18px;padding-bottom:12px;border-bottom:2px solid var(--teal-pale)}
...
.kl-dist-box{background:var(--card);border:1px solid;border-radius:8px;padding:9px 14px;
  display:flex;justify-content:space-between;align-items:center;box-shadow:0 1px 4px rgba(13,60,80,.06)}
/* Recommendations */
...
.rec-oi-box{background:var(--card);border:1px solid var(--bdr);border-radius:8px;padding:12px 14px;
  display:flex;align-items:center;gap:10px;flex-wrap:wrap;font-size:12px;box-shadow:0 1px 4px rgba(13,60,80,.06)}
...
.stab{padding:8px 18px;border-radius:8px;border:1.5px solid var(--tc,#0d8a9e);
...
.stab:hover{background:rgba(0,0,0,.05)}
.stab-active{background:var(--tc,#0d8a9e)!important;color:#fff!important;
...
.strat-card{background:var(--card);border:1px solid var(--bdr);border-radius:12px;
  padding:14px 12px 12px;cursor:pointer;transition:all .2s;box-shadow:0 1px 6px rgba(13,60,80,.07)}
...
/* Modal */
.smodal-overlay{display:none;position:fixed;inset:0;z-index:9000;
  background:rgba(13,30,40,.65);backdrop-filter:blur(4px);
  justify-content:center;align-items:center;padding:20px}
.smodal-box{background:var(--surf);border:1px solid var(--bdr2);border-radius:18px;
  width:100%;max-width:480px;box-shadow:0 24px 60px rgba(0,0,0,.3);overflow:hidden}
.smodal-header{display:flex;justify-content:space-between;align-items:flex-start;
...
.smodal-close{background:rgba(255,255,255,.15);border:none;color:#fff;width:30px;height:30px;
  border-radius:50%;cursor:pointer;font-size:14px;display:flex;align-items:center;justify-content:center;transition:background .15s}
.smodal-close:hover{background:rgba(255,255,255,.28)}
.smodal-body{padding:20px 22px}
.smodal-svg-wrap{display:flex;justify-content:center;margin-bottom:14px;
  background:var(--card);border-radius:10px;padding:14px;border:1px solid var(--bdr)}
...
/* Strikes table — wider columns now that we show OI Chg + Vol */
...
.s-table th{background:linear-gradient(135deg,#0d4a5a,#0d8a9e);color:#fff;padding:9px 10px;
  font-size:10px;font-weight:600;text-align:left;letter-spacing:.5px;white-space:nowrap}
.s-table td{padding:9px 10px;border-bottom:1px solid var(--bdr);font-size:11px;color:var(--text);background:var(--card)}
.s-table tr:last-child td{border-bottom:none}
.s-table tr:hover td{background:var(--surf)}
footer{padding:16px 32px;border-top:2px solid rgba(255,255,255,.12);
  background:linear-gradient(135deg,#0d4a5a,#0d6e80);display:flex;justify-content:space-between;
...
def generate_html(tech, oc, md, ts):
    ...
    n_str  = oc.get("total_strikes", "—") if oc else "—"

    bias = md["bias"];  conf = md["confidence"];  bull = md["bull"];  bear = md["bear"];  diff = md["diff"]
...
<div class="hero">
  ...
  <div class="hero-stats">
    ...
    <div class="hstat"><div class="hstat-lbl">Strikes</div><div class="hstat-val" style="color:#a8eee0;">{n_str}</div></div>
  </div>
</div>
...
def main():
    ...
    print("\n[1/3] Fetching NSE Option Chain (ALL strikes)...")
    ...
    if oc_analysis:
        print(f"  OK  Spot={oc_analysis['underlying']:.2f}  ATM={oc_analysis['atm_strike']}"
              f"  Strikes={oc_analysis['total_strikes']}  PCR={oc_analysis['pcr_oi']:.3f}")
    else:
    ...
    meta = {
        ...
        "total_strikes":  oc_analysis["total_strikes"] if oc_analysis else None,
