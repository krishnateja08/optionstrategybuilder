...
window.switchExpiry = function(exp) {{
  ...
  initAllCards();
  ['bullish','bearish','nondirectional'].forEach(sortGridByPoP);
  filterStrat(typeof _activeStratCat !== 'undefined' ? _activeStratCat : 'bullish', null);
  // Flash indicator
  const sel = document.getElementById('expiryDropdown');
  if (sel) {{
        ...
        setTimeout(function() {
          try {
            if (typeof initAllCards === 'function') {
              ...
              var activeCat = (typeof _activeStratCat !== 'undefined') ? _activeStratCat : 'bullish';
              if (typeof filterStrat === 'function') filterStrat(activeCat, null);
            }
...
<div class="main">
  <aside class="sidebar">
    ...
    <div class="sb-sec">
      ...
      <button class="sb-btn" onclick="go('srAnalyzerWrap',this)">&#9733; S/R Analyzer</button>
  ...
  <main class="content">
    ...
    {strat_html}
    <div id="srAnalyzerWrap">{sr_html}</div>
...
# =================================================================
#  SR ANALYZER SECTION (injected by patch_sr_analyzer.py)
# =================================================================
...
<div class="section" id="srAnalyzer">
  <div class="sec-title" style="color:#f5c518;border-color:rgba(245,197,24,.18);">
    &#9733; S/R OPTIONS ANALYZER
    <span class="sec-sub">Enter your weekly Support &amp; Resistance &rarr; get spread recommendation</span>
  </div>

  <!-- INPUT FORM -->
  <div style="display:flex;align-items:flex-end;gap:14px;flex-wrap:wrap;margin-bottom:20px;
              background:linear-gradient(135deg,rgba(245,197,24,.08),rgba(100,128,255,.06));
    ...
    <div style="align-self:flex-end;font-family:'DM Mono',monospace;font-size:11px;
      ...
      &nbsp;&nbsp;Lot: <span style="color:rgba(255,255,255,.5);">{lot_size}</span>
    </div>
  </div>
...
(function() {{
  ...
  window.srAnalyze = function() {{
    ...
    if (sup >= res) {{
      alert('Support must be less than Resistance.');
      return;
    }}
    ...
    else if (position >= 0.70) {{ posLabel = 'Near RESISTANCE'; posColor = '#ff6b6b'; }}
    else                       {{ posLabel = 'MIDDLE of Range'; posColor = '#6480ff'; }}
    ...
    setTimeout(function() {{
      ...
      var srWrap = document.getElementById('srAnalyzerWrap');
      if (contentDiv && srWrap) {{
        contentDiv.scrollTo({{ top: srWrap.offsetTop - 20, behavior: 'smooth' }});
      }} else if (srWrap) {{
        srWrap.scrollIntoView({{ behavior: 'smooth', block: 'start' }});
      }}
    }}, 60);
  }};
